class AuditLog
  def self.record!(subject:, event_type:, message:, user: nil, organization: nil, metadata: {})
    organization ||= subject.try(:organization) ||
      subject.try(:submission)&.organization ||
      subject.try(:policy)&.organization ||
      subject.try(:quote)&.submission&.organization ||
      subject.try(:business)&.submissions&.first&.organization
